...
//	fo->Close();
	...
	if(hd!=0){
	  ...
	  hd->Draw("hist p l same");
        ...
        l->AddEntry(hd,"Raw","l");
        l->AddEntry(h,"Raw-Pedestal","l");
        l->AddEntry(hb,"2015 Baseline","l");
        l->AddEntry(hc,"2015 Clusters","l");

	TH1F* he = (TH1F*) f->Get(dir[5]+"/"+obj->GetName());
	if(he!=0){
          h1->Add(he,-1);
	  h1->SetLineWidth(1);
	  h1->SetLineStyle(1);
	  ...
	  h1->Draw("hist l same");
        ...
        l->AddEntry(h1,"New Effective Baseline","l");
...
void displayClusters(TString file){
  ...
  l->AddEntry(h1[0],"2015 Algo","l"); 
  l->AddEntry(h2[0],"New Algo","l"); 
  l->Draw("same");
  ...
  h2[1]->GetXaxis()->SetRangeUser(0,50000);
  c1->SetLogy();
  h2[1]->SetStats(0);
  h2[1]->Draw();
  h1[1]->Draw("same");
  ...
  h2[2]->GetXaxis()->SetRangeUser(0,100);
  c1->SetLogx(0);
  h2[2]->SetStats(0);
  h2[2]->Draw();
  h1[2]->Draw("same");
  ...
  TH1F * m[10];
  m[0] = (TH1F*)f->Get("clusterMatching/nOldClusters");
  m[1] = (TH1F*)f->Get("clusterMatching/nMatchedClusters");
  m[2] = (TH1F*)f->Get("clusterMatching/nSplitClusters");

  TProfile * p[10];
  p[0] = (TProfile*)f->Get("clusterMatching/matchingMult");
  p[1] = (TProfile*)f->Get("clusterMatching/matchedWidth");
  p[2] = (TProfile*)f->Get("clusterMatching/matchedCharges");
  ...
  c1->SetLogy(0);
  m[1]->SetStats(0);
  m[1]->SetLineColor(kRed);
  m[1]->GetXaxis()->SetRangeUser(0,60);
  m[1]->Draw();
  m[2]->SetLineColor(kBlue);
  m[2]->Draw("same"); 
  l->Clear();
  l->SetY1(0.1);
  l->SetY2(1);
  l->AddEntry(m[1],"Matching Efficiency","l");
  l->AddEntry(m[2],"% Clusters Split","l");
  l->Draw("same");
  c1->SaveAs("img/ClusterMatchingEff.png");
  c1->SaveAs("img/ClusterMatchingEff.pdf");

  c1->Clear();
  p[0]->SetMarkerStyle(8);
  p[0]->SetLineColor(kBlack);
  p[0]->GetXaxis()->SetRangeUser(0,60);
  p[0]->SetStats(0);
  p[0]->Draw();
  c1->SaveAs("img/SplitMultiplicities.png");
  c1->SaveAs("img/SplitMultiplicities.pdf");
  c1->Clear();
  p[1]->SetMarkerStyle(8);
  p[1]->SetLineColor(kBlack);
  p[1]->GetXaxis()->SetRangeUser(0,60);
  p[1]->SetStats(0);
  p[1]->Draw();
  c1->SaveAs("img/WidthRatios.png");
  c1->SaveAs("img/WidthRatios.pdf");
  p[2]->SetMarkerStyle(8);
  p[2]->SetLineColor(kBlack);
  p[2]->GetXaxis()->SetRangeUser(0,60);
  p[2]->SetStats(0);
  p[2]->Draw();
  c1->SaveAs("img/ChargeRatios.png");
  c1->SaveAs("img/ChargeRatios.pdf");
}
