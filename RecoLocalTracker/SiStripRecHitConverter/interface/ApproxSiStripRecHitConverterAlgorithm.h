...
public:
  ...
  ApproxSiStripRecHitConverterAlgorithm(const edm::ParameterSet&, edm::ConsumesCollector);
  void initialize(const edm::EventSetup&);
  void run(edm::Handle<edmNew::DetSetVector<SiStripApproximateCluster> > input, products& output);
  void run(edm::Handle<edmNew::DetSetVector<SiStripApproximateCluster> > input, products& output, LocalVector trackdirection);
...
private:
  ...
  bool isMasked(const SiStripApproximateCluster& cluster, bool bad128StripBlocks[6]) const;
