...
ApproxSiStripRecHitConverterAlgorithm::ApproxSiStripRecHitConverterAlgorithm(const edm::ParameterSet& conf,
...
void ApproxSiStripRecHitConverterAlgorithm::fillPSetDescription(edm::ParameterSetDescription& desc) {
...
void ApproxSiStripRecHitConverterAlgorithm::initialize(const edm::EventSetup& es) {
...
void ApproxSiStripRecHitConverterAlgorithm::run(edm::Handle<edmNew::DetSetVector<SiStripApproximateCluster> > input, products& output) {
  run(input, output, LocalVector(0., 0., 0.));
}

void ApproxSiStripRecHitConverterAlgorithm::run(edm::Handle<edmNew::DetSetVector<SiStripApproximateCluster> > inputhandle,
                                          products& output,
  ...
  for (auto const& DS : *inputhandle) {
    ...
    for (auto const& cluster : DS) {
      ...
      StripClusterParameterEstimator::LocalValues parameters = parameterestimator->approxlocalParameters(cluster, du);
      collector.push_back(
...
void ApproxSiStripRecHitConverterAlgorithm::match(products& output, LocalVector trackdirection) const {
  int nmatch = 0;
...
void ApproxSiStripRecHitConverterAlgorithm::fillBad128StripBlocks(const uint32_t detid, bool bad128StripBlocks[6]) const {
...
inline bool ApproxSiStripRecHitConverterAlgorithm::isMasked(const SiStripApproximateCluster& cluster, bool bad128StripBlocks[6]) const {
  return false;
}

inline bool ApproxSiStripRecHitConverterAlgorithm::useModule(const uint32_t id) const {
  const StripGeomDetUnit* stripdet = (const StripGeomDetUnit*)tracker->idToDetUnit(id);
  if (stripdet == nullptr)
    edm::LogWarning("ApproxSiStripRecHitConverter") << "Detid=" << id << " not found";
